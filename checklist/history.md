...
### DB 정합성 — 클린
- 참조 무결성: mps/heatmap/player_stats → players/events 고아 **0**. (단 `teams` 부재 team_id 소속 선수 7명 — 전원 경기데이터 0, 영향 미미)
- 중복 0, 값 범위 위반 0(평점 0~10·출전 0~120·히트맵 좌표).
- `result` 컬럼 = 2승/1무/0패 인코딩, 실제 스코어와 **99.5%+ 일치**(이상치 0.3%는 연기·몰수 추정).
- `name_ko` 깨짐(`�`) **0** — 앞선 "깨짐" 인상은 콘솔 cp949 표시 아티팩트였고 실제 DB는 정상 한글. 영문 name_ko 95건은 외국인/미번역(손상 아님).
- 점수 NULL 262 = 미래 예정 259(정상) + 과거 3건(연기/몰수 추정).

### 페이지 오류 스윕 (Playwright, 콘솔 error·HTTP 5xx·reqfail 수집)
- ✅ 에러 0: `/` 초기로드, 5탭(전술판·예측·팀·선수·히트맵), 팀(단일/비교), 선수(인사이트·드로어·선수비교), 매치선택(K1·경기클릭), 경기예측, `/login`.
- ❌ **`/leagues` 1곳만 HTTP 500** — `/api/league/epl/standings`가 빈 `epl.db`(0바이트, 테이블 없음)에서 `no such table: events`. 해외리그 DB 미수집 탓. UI 진입점은 이미 비활성화돼 일반 탐색 도달 불가하나 라우트/API는 생존 → 직접 접근 시 500. (가드 적용 보류 — 후속 결정)

---

## 2026-06-17 (후속) | 🔧 선수 리포트 — 생년+등번호 폴백 매칭으로 20명 구제

후속 검증(사용자: "한번 확인해줘"): 보드 리포트 실패 66명을 dob+등번호로 정밀 대조.
- **20명은 데이터 부재가 아니라 이름 매칭 누락**이었음 — 깨진 `name_ko`/영문전용 레코드라 한글명 매칭 실패하나 실제로는 경기 데이터 보유(박준서 54경기·정명재 53·이규백 41·정승배 37 등).
- 나머지 45명은 진짜 부재(2003~2008년생 유스, SofaScore 출전기록 없음 → 수집 불가).

### 수정 (오매칭 위험 0 설계)
- **`main.py` `_find_player_by_dob_shirt`** 추가 + `player-stat-report` 폴백: 이름 실패 시 dob(±1일, 타임존 보정) 후보 중 ①등번호 일치 1명 → 채택, ②등번호 불일치여도 dob 후보 정확히 1명 → 채택, ③모호하면 None. 등번호는 소스마다 달라(박준서 DB20 vs 스쿼드45) 단독 신뢰 불가 → dob 주신호.
- **`app.js`**: 보드 `openPlayerReport`에 `dob`·`shirt`(등번호) 동봉. `applySquad`가 보드 선수에 dob·number 보존 확인.
- **`player_report.js`**: dob·shirt 수신 → URL 동봉(이름 매칭 시엔 무시 → 기존 선수 영향 0).
- 실측: 미매칭 65명 → 폴백 후 **found:True 20명 / 데이터없음 45명 / 오매칭 0**. 브라우저 체인(박준서) 풀 리포트 렌더 확인.
...
### 수정 (모두 프론트, MIN=4 / vs-teams는 2)
- **월별 승률**(analytics renderMonth): <4경기 월 막대 **회색** + 툴팁 "⚠표본 적음".
- **상대팀별 전적**(renderVsOpponents): 승률 정렬이 1경기 100%를 "만만한 상대" 최상단에 올리던 문제 → **표본 충분(≥4) 상대 먼저 정렬**, <4 상대 승률 배지 **점선·흐리게** + 하단 안내행. (단일시즌은 상대당 1경기뿐이라 전부 약화 — 정직.)
- **선수 상대팀별 성적**(player_report renderVsTeams): 평점순 "좋은 상대"인데 1경기 고평점이 최상단 → ≥2경기 먼저 정렬, 1경기는 평점 흐리게+`*` 각주+안내.

### 점검만(가드 이미 있음/안전)
- 인사이트 날씨 temp_buckets: **리그 전체 25~91경기**(안전). 공중볼 승률: "최소 표본 30" 필터. per-90: "5경기·450분↑". → 추가 조치 불필요.
- 홈/원정·선제골: 보통 표본 충분(저위험) → 보류.

### 검증
- Playwright: 상대팀별(수원 2026) 1경기 상대 전부 [소표본] 약화+안내행, 콘솔 에러 0. 월별 회색막대 로직 동일.
- `analytics.js` v15→16, `player_report.js` v6→7, `style.css` v95→96. 백엔드 무변경.

---

## 2026-06-15 | 🔧 날씨별 승률 표본 함정 가드 (renderWeather)

### 문제 (사용자 제보: "데이터 오류 함정 느낌")
- 집계 자체는 정상(`GROUP BY e.id` 경기당·정규화 team_id, 커버리지 100%). 함정은 **표본 크기**: 시즌 14경기를 온도 4구간으로 쪼개면 영하~5도=1~2경기 → "승률 100%(1-0-0)"가 6경기 구간과 시각적으로 동등하게 표시돼 신뢰 불가 수치가 사실처럼 보임.

### 수정 (analytics.js renderWeather)
- 구간 경기수 < **4**면 `ta-wx-low`: 막대 회색 + 승률 흐리게 + **W-D-L 대신 "N경기"만**(주황) 표기 → 수치 오해 방지. 하단 ⚠ 안내("4경기 미만은 참고용, 시즌 진행될수록 정확").
- 검증: Playwright(수원삼성) — 영하~5도 "100% 2경기"가 표본부족으로 약화, ≥4경기 구간만 정상 W-D-L, 콘솔 에러 0.
- 백엔드 무변경(프론트만). `analytics.js` v14→15, `style.css` v94→95.

---

## 2026-06-15 | ✅ 선수 데이터 정합성 종합 감사 (읽기 전용 판단)

포지션 수정 후 "이 클래스 또 있나" 사용자 요청 → mps 69,622행 전수 감사.

### 핵심 무결성: 전부 클린
- mps.team_id = 이벤트 유도 팀(is_home): **0 불일치** (과거 normalize_mps_team 유지)
- 스탯 값 이상치: **0** (분[0,130]·패스%[0,100]·평점(0,10]·음수·goals≤shots·sot≤shots)
- tournament_id: 410/777만 (K3·잡음 없음) · (경기,선수) 중복행 **0** · 고아 player_id **0**

### 잔여 2건 — 현재 시즌 영향 0 (조치 불필요)
- **name_ko 없는 선수 175명**: 전부 **2026 미출전(과거/은퇴: 이동국·염기훈 등 + 외국인)** → 현 시즌 검색·로스터·랭킹 노출 0. 로컬에 한글명 소스도 없음(전부 로마자).
- **동일이름+동일팀 중복 ID**: 314건 대부분 시즌 다른 동명이인. 같은 2026·같은 팀으로 좁히면 9건, 그중 7건은 같은 경기 동시출전(=다른 선수), 2건(디자우마 등)만 ID 분절 의심 — 의심 id당 1~2경기라 영향 미미.

### 판단
**선수 데이터 정합성 완료(클린).** 분석 정확성 영향분(스코어 반전·포지션)은 기수정. 잔여는 현지화 표기+극소수 빈 ID로 저위험. **변경/배포 없음(읽기 전용).**

---

## 2026-06-15 | 🔧 포지션 분류 정합성 통일 — 세부그룹 우선(윙어=공격)

### 발견 (사용자 제보: "공격수인데 미드필더로 분류")
- 전수 감사: 2026 5경기+ 선수 중 **133명**이 `mps.position` 대분류 ↔ `detail_pos` 세부그룹 대분류 불일치.
- 지배 패턴 **미드필더(mps) → 공격수(세부) 98명** — SofaScore가 윙어를 M으로 묶는데, 우리 세부(W→F)는 공격으로 분류. 화면이 어느 소스를 쓰냐에 따라 같은 선수가 미드/윙어로 갈림. (에드가 본인은 전 소스 F로 일치 — 사용자가 본 건 이 98명 중 윙어였을 가능성.)

### 수정 `player-stat-report` — eff_pos(세부 우선) 도입
- `eff_pos = DETAIL_TO_BROAD.get(detail_grp) or pos_raw` — 세부그룹 있으면 그 대분류, 없으면 mps 폴백.
- 적용: 포지션 라벨(pos_label)·지표 세트(POS_GROUPS)·레이더(RADAR_KEYS)·비교군 라벨 폴백. 비교군 풀 쿼리는 pos_raw(mps) 유지(풀 멤버십 안정).
- **부수 버그 수정**: `pos_label`이 폴백 분기(4279)에서 정의(4486) 전에 참조되던 UnboundLocalError 잠복 버그 → eff_pos와 함께 일찍 정의.
- 반환 `player.pos`도 eff_pos로 통일(compare 레이더 축 선택 등 정합).
- 검증: 윙어(송민규·이동경·문선민·이청용) → pos=F·FW·윙어 + 공격 지표세트(득점·슈팅·드리블). 회귀: 에드가=FW·스트라이커, 이케르=MF·중앙MF(유지), detail 없는 선수 폴백 HTTP 200, `qa_check.py` 31/31 PASS.
- 인사이트/팀형태는 이미 세부 기반(DETAIL_BROAD)이라 변경 불필요. **DB 무변경·백엔드만 → 자산버전 불필요, 서비스 재시작으로 캐시 갱신**.

---

## 2026-06-15 | 🔴🔧 events 스코어 12경기 좌우반전 교정 (마스터 결과 데이터)

### 발견 (QA 중, 경기리포트가 잠복버그 가시화)
- 7인+PM 데이터 QA에서 ⑤경기리포트 골 타임라인이 스코어와 안 맞는 경기 발견 → 전수 조사.
- **events.home_score↔away_score가 12경기에서 좌우반전**(SofaScore 크롤 단계 유입, sync는 'score 있으면 SKIP'이라 못 고침).
- **3중 교차검증**: ① 슛맵/골기록(SofaScore) 12경기 모두 스코어의 정확한 반전 ② K리그 공식 결과 JSON 210경기 대조 → 딱 12경기 불일치, 전부 정확한 반전(198경기 정상) ③ 외부 뉴스(울산 1-4 대전·김천 0-3 강원) → JSON·슛맵이 진실.
- 영향: 순위표·전적·팀분석·예측백테스트가 이 12경기 잘못된 결과 사용 중이었음. 오늘 코드가 만든 버그 아님(잠복 데이터 오류).

### 교정 (승인 후)
- DB 백업(`players_pre_score_fix_*.db`) → `crawlers/fix_swapped_scores.py`(신규, 공식 JSON 기준 멱등 교정) `--apply`로 12경기 정정. 재실행 0건(멱등) 확인.
- **재발 방지**: `sync_results_to_events.py` 패치 — 기존 score도 **공식 JSON과 다르면 갱신**(현재 NULL만 채움). 운영은 다음 `update_data.py`(STEP sync) 때 자가치유. 패치 후 sync 재실행 = 0 업데이트(이미 정확, 대량 덮어쓰기 없음 확인).
- 검증: 12경기 events==슛맵 12/12 일치 · 순위표 API==events 재계산(대전 W4D4L7·강원 W6D6L3 등) 정확 · pts=w*3+d 무결성 12/12 · `qa_check.py` 31/31 PASS(예측 회귀 없음).
- 12경기: FC서울-대전, 울산-대전, 강원-대전, 강원-FC서울, 김천-강원, 부천-김천, 광주-FC서울, 용인-김해, 대구-천안, 파주-경남, 부산-수원FC, 성남-파주.
- 운영 DB도 동일 버그 존재 → SSH로 백업+`fix_swapped_scores.py --apply`+서비스 재시작(캐시 클리어)로 교정.

---

## 2026-06-15 | ✅ 통합 점검(QA) — 오늘 출시 4종 실브라우저·모바일·엣지·회귀

### 범위
- 오늘 출시: ③스카우팅 ⑤경기리포트 ④선수비교 + K1 선수 상세 지원. C1(QA) 관점 종합 점검.

### 점검 결과
- **자체 QA 스윕 14/14 PASS** (Playwright): 초기로드 콘솔0 · 스카우팅 렌더 · 경기리포트(정상+슛맵없는 경기 graceful) · 선수비교 K1↔K2 배지·15지표바 · 접근성(닫기 title·ESC) · 모바일(390px) 가로 오버플로 ≤8px · 모바일 콘솔0.
- **프로젝트 스모크 `qa_check.py` 31/31 PASS (0 FAIL)** — K1 일반화로 인한 API 회귀 없음 확인.

### 발견·수정 (P6 모바일 폴리시 1건)
- 경기리포트 헤더 스코어가 390px에서 세로로 줄바꿈(1/:/2)되고 원정 팀명 잘림 → `.mr-score-num{white-space:nowrap}` + `@media(max-width:720px)` 폰트 축소(스코어 26px·팀명 14px). 한 줄 복원 확인.
- `style.css` v93→94, `index.html`(버전). 그 외 변경 없음.

---

## 2026-06-15 | 🔧 선수 상세 스탯·비교 K1 지원 — player-stat-report 리그 일반화

### 배경 (PM 권고 1순위: 이미 만든 선수 기능의 도달 범위 2배)
- 선수 리포트·⚖️ 선수 비교·퍼센타일이 전부 K2 전용(tid=777 하드코딩 8곳)이라 간판 리그 K1을 못 봤음. mps에 K1(410) 데이터는 이미 존재 → 신규 수집 0.

### 백엔드 `player-stat-report` 일반화 (P4 Red — 신중)
- **리그 자동 판정**: 선수의 가장 '최근' 경기 대회(410/777)로 tid 결정. `heatmap-player-search`가 MAX(date_ts)로 매기는 배지와 일치시켜 이적 선수도 현 소속 리그로 귀속(처음엔 '최다 출전'으로 했다가 일류첸코 K1(119)/K2(91) 불일치 발견 → '최근'으로 교정).
- 하드코딩 8곳(출전포지션·연도·대상집계·신체랭크 EXISTS·detail_groups·피어집계·활동량·최근폼) 전부 `tid` 파라미터 바인딩으로 치환 → 비교군·백분위·활동량 모두 해당 리그 내 계산(통계 정합). 응답에 `league` 추가.
- **검증**: K1 선수(146 K1 피어·센터백 비교군)·K2 회귀(일류첸코 91경기 40골 동일·129 피어)·둘 다 뛴 선수 최근 리그 귀속 PASS.

### 프론트 `player_compare.js` v2
- 검색 K2 필터 제거(K1+K2) + 결과·헤더에 리그 배지(K1 파랑/K2 노랑). 교차 리그(K1↔K2) 비교 시 레이더 주석 "백분위는 각자 자기 리그·포지션 내 기준(참고용)".
- 단일 선수 리포트(`player_report.js`, 이름 호출)도 같은 백엔드라 K1 자동 지원(보너스).
- 검증: Playwright K1(전북 D) vs K2(일류첸코) — 배지 K1/K2·15지표바·레이더 paint·교차주석·콘솔 에러 0.
- **DB 무변경(읽기 전용) → prod 마이그레이션 불필요**. `main.py`(tid 일반화), `player_compare.js` v1→2, `style.css` v92→93, `index.html`(라벨·버전).
...
### 후속(같은 날): 슛맵 정제 — 결과 필터 + 요약 상세화
- 피드백: 설명 부족, 색상(골/유효 등) 필터 필요, 슈팅 요약 가독성·설명 개선.
- **결과 필터 칩**(`ta-sm-chip`): 골/유효(선방)/골대/차단/빗나감 클릭 토글 + 개수 표시, 끄면 캔버스서 제외 + 숨김 개수 노트. `_shotFilter` 상태.
...
### 후속3(같은 날): 누적 xG → 경기당 지표로 (유의미성 교정)
- 피드백(P4): "누적 xG가 큰데 유의미한가" → 맞음. 누적 xG는 경기 수 비례 볼륨 지표라 비교 부적합(특히 기본값 '전체'=다시즌 누적).
- 요약 전면을 **경기당 지표**로: 경기당 슈팅·경기당 득점·**경기당 xG**(누적·슛당은 보조 표기). 백엔드 summary에 `xgPerGame`·`gpgPerGame` 추가. 예: 안산 2026 경기당 xG 1.16 vs 전체 0.75(누적 16.18·83.83은 경기수 차이일 뿐).
- 검증: Playwright 경기당 라벨·콘솔 에러 0. `analytics.js` v10→11.

### 후속4(같은 날): 슛맵 경기당 지표 리그 평균 대비 + 분모 정합 수정
- 요청: 리그 평균 경기당 xG 대비 추가.
- 🔴 **정합 수정**: 슛맵 커버리지 ~45%인데 경기당 지표를 '전체 경기'로 나눠 과소평가 → **'슛 보유 경기'(distinct shotmap event) 기준**으로 분모 교정.
- **리그 평균 baseline**: 같은 리그·시즌 전체 슛 1회 집계 → 팀·경기당 평균 = 합계/(2×슛보유경기). summary.league(perGame·gpgPerGame·xgPerGame).
- 프론트: 경기당 슈팅·득점·xG 카드에 **▲▼ vs 리그 N**(for=높을수록 좋음, against=낮을수록 좋음 색 반전). 예: 안산 2026 경기당 xG 1.16 vs 리그 1.39 ▼(공격 평균 이하 — 수비 블록과 일치).
- 검증: Playwright 리그 대비 표시·콘솔 에러 0. `analytics.js` v11→12.
...
### 발견 (다른 리그 확장 논의 중)
- `teams`에 `league='K3리그'`(tid=10268) 9팀 + 등록 선수 61명. 처음엔 "지울 K3 잡데이터"로 보였으나 조사 결과 **삭제하면 K2 데이터 손상**.
- 정밀 진단: 61명 중 **58명이 실제 K1/K2 선수**인데 `mps.team_id`가 '현 소속(K3 강등 구단: 시흥시민·부산교통공사·창원시청 등)'으로 고정 → 과거 K2 경기 **804건이 K3팀으로 잘못 귀속**. mps.team_id가 실제 경기 양팀과 **100% 불일치**(0/804). 유도된 실제 소속 = 안산·성남·포항·부천·서울·전남·천안·수원삼성 등.
- **원인 = 헤이스(K1→K2)와 동일한 mps.team_id 현소속고정 버그**, 여기선 K2→K3 강등 케이스. 히트맵 커리어 뷰는 event 기반이라 영향 없었지만 **팀별 스탯 집계는 실 K2팀을 과소집계**.

### 처리 — `crawlers/fix_k3_mistag.py`(멱등, 단일 트랜잭션, 백업 후)
- **② 교정**: mps.team_id 804건을 event(home/away)+is_home 유도 실제팀으로 재매핑, players.team_id 58명을 교정 mps 최빈팀으로(예: Wan-kyu Kwon→포항, 박태용→전남).
- **① 삭제**: K3 대회 13경기 + 히트맵 935점 + 순수 K3 선수 3명(데이터 0) + K3 팀 9개. 안전장치: 비-K3 이벤트가 K3팀 참조 시 중단, 삭제 직전 잔여참조 0 확인.
- **검증**: K3 잔여 0. 고아 mps team_id 백업=현재 **15,488 동일**(새 고아 미생성), players 고아 **682 동일**. 스모크 standings/ranking/top-performers/teams 전부 200, 검색 'Siheung' 0건.
- ※ 별개 기존 이슈로 **team_id가 teams 마스터에 없는 mps 15,488·players 682건** 존재(여러 대회 잔재) — K3와 무관, 추후 검토.
...
### 히트맵 팀 그리드 정리 — 현 시즌 12/17로 (과거 시즌 잔재 제거)
- **문제**: `_heatmap_teams_for_league`가 해당 토너먼트에 '과거에라도 등장한 모든 팀'을 반환 → K1 그리드 36팀(2020~2025 강등/승격 잔재), 현 K1은 12팀.
- **수정**: `TEAMS.league`(현 시즌 소속) 기준 + 데이터 보유 교차 필터로 변경 → K1 **12**, K2 **17**, `/api/teams`와 정확히 일치. 교차 리그 잔재는 league 필터가 처리.
...
### 년도(시즌) 필터 상시 노출 + "시즌" 라벨
- **배경**: 년도 필터(6d23da8)가 다중 시즌 선수에만 노출(`seasons.length<=1`이면 숨김) → 2026만 있는 현역 선수에선 안 보여 "필터 없다"는 피드백.
- **변경**: 임계 `<1`로 완화(데이터 0일 때만 숨김) + "시즌" 라벨 추가 → 단일 시즌도 `시즌 [전체][2026]` 노출. k2heatmap v7→8·style v78→79.
- **검증(브라우저)**: 엄원상(2026만) "시즌 전체 2026" 노출, 홍정호(6시즌) 전체+5시즌 노출, 에러 0. 참고: 강현무처럼 mps만 있고 heatmap_points 0인 선수는 빈 히트맵+필터 미노출(데이터 없음) — FC서울 29명 중 8명이 히트맵 무(향후 리스트에서 제외 검토 여지).

### 선수 목록 히트맵 보유만 + 경기장 크기 축소
- **선수 목록 정리**: `_heatmap_players_for_team` games를 '히트맵 보유 경기수' 서브쿼리로 + `HAVING games>0` → mps만 있고 heatmap_points 0인 선수(빈 히트맵) 제외. 검증: FC서울 29→21명, 강현무(GK 무데이터) 빠지고 구성윤(GK 데이터有) 노출. 평점 평균은 mps 기준 유지.
...
### 히트맵 점을 경기장 안쪽으로 매핑 (경기장보다 살짝 작게)
- **문제**: 히트맵 점이 캔버스 0~100%(0~W)로 찍히는데 경기장 라인은 5~95% → 히트맵이 경기장 밖까지 번져 경기장보다 커 보임.
...
### ⏸ 비교 필터 "포지션 세분화" — 데이터 제약 진단 후 보류(결정 대기)
- 사용자가 G/D/M/F → 세부 포지션(CB/FB·DM/AM 등) 비교 요청. 진단: **DB에 세부 포지션 라벨 없음**(mps·players·match_lineups·raw_json 전부 G/D/M/F). 두 안 제시 → 미결정.
  - A. 히트맵 centroid 추정(즉시·부정확) / B. SofaScore 세부 포지션 백필(정확·수집 필요).
- 재개 지점은 `RESUME.md §0`. **다음 세션은 A/B 선택부터.**
...
- **K리그2 6/5~6/7 8경기 수집** (대구-파주 등 3 / 6.6 2 / 6.7 3): 로컬 DB가 5/31에서 정체돼 미수집이던 상태.
  ...
  - 커밋 `599973b` → push → CI/CD 자동 배포. 로컬=origin=prod HEAD 일치, 서비스 active/200.
- **prod 정합화**: prod는 자체 수집으로 7경기 이미 보유, 천안–수원FC(`ev15403899`) 히트맵만 0건 → prod `venv/bin/python3 crawlers/fetch_event_heatmap.py`로 1,312 pts 보강.
...
### 수비P 정밀화 — 백필 + 산식 통합 (커밋 `36834b3`, `f86bb0d`)
- **백필**(`crawlers/backfill_duel_detail.py`, 신규): raw_json에서 `won_tackle`·`ball_recovery`·`challenge_lost` 추출(외부 API 불필요). 로컬+운영 각 68,330행. 크롤러(`backfill_k1_mps`·`crawl_match_stats`) 매핑+스키마 추가로 향후 자동 수집.
- **정밀 산식**: `수비P = (태클 + 인터셉트×1.5 + 클리어 + 슈팅차단 + 볼회수×0.5 − 피드리블)/90`. ball_recovery는 빈도 높아 ×0.5, challenge_lost 감점.
...
20
- 2026-05-19 12:26:43 | git log -1 --format="%s%n%n%b"
- 2026-05-19 12:26:43 | git diff --stat HEAD checklist/history.md | head -20
- 2026-05-19 12:26:47 | git diff HEAD checklist/history.md
- 2026-05-19 12:30:29 | python -c " / import sqlite3 / conn = sqlite3.connect('players.db') / cur = conn.cursor() / cur.execute('PRAGMA table_info(events)') / for r in cur.fetchall(): /     print(r) / "
- 2026-05-19 12:32:50 | python update_data.py 2>&1
- 2026-05-19 12:35:22 | grep -n "STEP\|def \|crawl_\|fetch_" update_data.py | head -50
- 2026-05-19 12:35:38 | grep -n "def \|--league\|WHERE\|MISSING\|already\|skip" crawlers/backfill_k1_mps.py | head -40
- 2026-05-19 12:38:04 | powershell -Command "Get-Content 'C:\Users\ehban\AppData\Local\Temp\claude\C--Users-ehban-OneDrive-------today-tatics\a6fc7be6-bc0a-4300-91b6-83c0a6b6469e\tasks\btwk4calj.output' -Tail 40"
- 2026-05-19 12:38:37 | powershell -Command "Get-Content 'C:\Users\ehban\AppData\Local\Temp\claude\C--Users-ehban-OneDrive-------today-tatics\a6fc7be6-bc0a-4300-91b6-83c0a6b6469e\tasks\btwk4calj.output' -Tail 30"
- 2026-05-19 12:46:58 | powershell -Command "Get-Content 'C:\Users\ehban\AppData\Local\Temp\claude\C--Users-ehban-OneDrive-------today-tatics\a6fc7be6-bc0a-4300-91b6-83c0a6b6469e\tasks\btwk4calj.output' -Tail 40"
- 2026-05-19 12:51:21 | powershell -Command "Get-Content 'C:\Users\ehban\AppData\Local\Temp\claude\C--Users-ehban-OneDrive-------today-tatics\a6fc7be6-bc0a-4300-91b6-83c0a6b6469e\tasks\btwk4calj.output' -Tail 20"
- 2026-05-19 12:57:14 | powershell -Command "Get-Content 'C:\Users\ehban\AppData\Local\Temp\claude\C--Users-ehban-OneDrive-------today-tatics\a6fc7be6-bc0a-4300-91b6-83c0a6b6469e\tasks\btwk4calj.output' -Tail 30"
- 2026-05-19 12:58:11 | python -c " / import json / with open('data/kleague_results_2026.json', encoding='utf-8') as f: /     d = json.load(f) / print('type:', type(d).__name__, 'len:', len(d)) / if isinstance(d, dict): /     print('top keys:', list(d.keys())[:5]) /     first_key = list(d.keys())[0] /     val = d[first_key] /     print(f'first key={first_key}, val type={type(val).__name__}') /     if isinstance(val, list) and val: /         print(f'  first item keys: {list(val[0].keys())[:10]}') /         print(f'  sample: {val[0]}')"
- 2026-05-19 12:58:38 | $env:PYTHONIOENCODING="utf-8"; python -c " / import json / with open('data/kleague_results_2026.json', encoding='utf-8') as f: /     d = json.load(f) / print('5/16~19 weekend matches in JSON:') / for team, matches in d.items(): /     for m in matches: /         dt = m.get('date','') /         if dt and dt[:10] in ('2026-05-16','2026-05-17','2026-05-18','2026-05-19'): /             home = 'H' if m.get('home') else 'A' /             print(f\"  {dt} | {team:<15} {home} vs {m.get('opponent','?'):<15} | {m.get('score','?'):<6} | {m.get('result','?')}\")"
- 2026-05-19 12:58:48 | grep -n "UPDATE\|INSERT\|home_score\|away_score\|date_ts\|score" crawlers/sync_results_to_events.py | head -40
- 2026-05-19 12:59:33 | grep -n "SLUG_META\|date_to_ts\|^[A-Z_]\+ *=" crawlers/sync_results_to_events.py | head -20
- 2026-05-19 13:36:06 | python -c " / import sqlite3 / from datetime import datetime / conn = sqlite3.connect('players.db') / cur = conn.cursor() / cur.execute('PRAGMA table_info(match_avg_positions)') / print('match_avg_positions:', [c[1] for c in cur.fetchall()]) / cur.execute('PRAGMA table_info(match_lineups)') / print('match_lineups:', [c[1] for c in cur.fetchall()]) / cur.execute('PRAGMA table_info(kleague_lineup)') / print('kleague_lineup:', [c[1] for c in cur.fetchall()]) / cur.execute('PRAGMA table_info(match_shotmap)') / print('match_shotmap:', [c[1] for c in cur.fetchall()]) / "
- 2026-05-19 13:36:20 | python -c " / import sqlite3 / from datetime import datetime / conn = sqlite3.connect('players.db') / cur = conn.cursor() / cur.execute(''' / SELECT e.id, e.tournament_id, e.date_ts, e.home_team_name, e.away_team_name, /        e.home_score, e.away_score, /        (SELECT COUNT(DISTINCT player_id) FROM match_lineups l WHERE l.event_id = e.id) as lu, /        (SELECT COUNT(*) FROM match_player_stats m WHERE m.event_id = e.id) as mps, /        (SELECT COUNT(*) FROM heatmap_points h WHERE h.event_id = e.id) as hp, /        (SELECT COUNT(*) FROM match_avg_positions a WHERE a.event_id = e.id) as ap, /        (SELECT COUNT(*) FROM match_shotmap s WHERE s.event_id = e.id) as sm, /        (SELECT COUNT(*) FROM kleague_lineup k WHERE k.sofa_event_id = e.id) as kl, /        (SELECT COUNT(*) FROM goal_events g WHERE g.event_id = e.id) as ge, /        (SELECT COUNT(*) FROM card_events c WHERE c.event_id = e.id) as ce / FROM events e / WHERE e.date_ts >= ? AND e.date_ts < ? / ORDER BY e.tournament_id, e.date_ts / ''', (int(datetime(2026,5,16).timestamp()), int(datetime(2026,5,18,23,59).timestamp()))) /  / print(f'{\"EID\":<10} {\"T\":<5} {\"DATE\":<11} {\"MATCH\":<48} {\"SC\":<6} {\"LU\":>3} {\"MPS\":>4} {\"HEAT\":>5} {\"AVG\":>4} {\"SHOT\":>5} {\"KL\":>3} {\"G\":>3} {\"C\":>3}') / print('-'*125) / for r in cur.fetchall(): /     eid, tid, ts, ht, at, hs, asc, lu, mps, hp, ap, sm, kl, ge, ce = r /     dt = datetime.fromtimestamp(ts).strftime('%m-%d %H:%M') /     score = f'{hs}-{asc}' if hs is not None else 'TBD' /     match = f'{ht[:22]} vs {at[:22]}' /     print(f'{eid:<10} T{tid:<4} {dt:<11} {match:<48} {score:<6} {lu:>3} {mps:>4} {hp:>5} {ap:>4} {sm:>5} {kl:>3} {ge:>3} {ce:>3}') / "
- 2026-05-19 13:36:52 | grep -n "def \|--days\|--league\|NOT EXISTS\|WHERE\|already\|target_events\|where" crawlers/crawl_lineups.py | head -30
- 2026-05-19 13:37:32 | python crawlers/crawl_lineups.py --days 4 2>&1
- 2026-05-19 13:38:08 | python crawlers/backfill_k1_mps.py --league K1 && python crawlers/backfill_k1_mps.py --league K2 && python crawlers/fetch_event_heatmap.py --days 4 && python crawlers/fetch_match_extras.py --days 4 --league all
- 2026-05-19 13:51:02 | python main.py 2>&1
- 2026-05-19 13:51:08 | until curl -s http://localhost:5000/api/teams -o /dev/null -w "%{http_code}\n" 2>/dev/null | grep -q "200"; do sleep 1; done; echo "[Flask Ready]"
- 2026-05-19 13:51:13 | curl -s "http://localhost:5000/api/player-matches?event_id=15373013" -o nul -w "player-matches: %{http_code}\n" / curl -s "http://localhost:5000/api/heatmap?event_id=15373013&player_id=0" -o nul -w "heatmap: %{http_code}\n"
- 2026-05-19 13:51:18 | grep -nE "def +(api_results|api_heatmap|player_matches|broadcast|api_lineup|api_player_matches)" main.py | head -20
- 2026-05-19 13:51:22 | grep -nE "@app.route" main.py | head -30
- 2026-05-19 13:51:30 | grep -nE "@app.route.*(broadcast|formation|lineup|avg_position|player-match|player_match|heatmap|match)" main.py | head -25
- 2026-05-19 13:51:39 | curl -s "http://localhost:5000/api/match-lineup?event_id=15373013" -o nul -w "match-lineup: %{http_code}\n" / curl -s "http://localhost:5000/api/match-extras?event_id=15373013" -o nul -w "match-extras: %{http_code}\n" / curl -s "http://localhost:5000/api/match-retrospective?event_id=15373013" -o nul -w "match-retrospective: %{http_code}\n" / curl -s "http://localhost:5000/api/matches-by-date?date=2026-05-17" -o nul -w "matches-by-date: %{http_code}\n"
- 2026-05-19 13:51:47 | curl -s "http://localhost:5000/api/match-lineup?event_id=15373013" | python -c " / import sys, json / d = json.load(sys.stdin) / print('keys:', list(d.keys())[:15]) / print('home formation:', d.get('home',{}).get('formation'), '| starters:', len(d.get('home',{}).get('starters',[]))) / print('away formation:', d.get('away',{}).get('formation'), '| starters:', len(d.get('away',{}).get('starters',[]))) / print('home sample:', d.get('home',{}).get('starters',[])[:2]) / "
- 2026-05-19 14:01:53 | python crawlers/sync_results_to_events.py 2>&1 | tail -5
- 2026-05-19 14:02:30 | git status; echo ---; git diff --stat
- 2026-05-19 14:02:49 | python -c " / import re / p = 'checklist/history.md' / with open(p, encoding='utf-8') as f: t = f.read() / n1 = t / n1 = re.sub(r'1\.201\.126\.200', '<<DEPLOY_IP>>', n1) / n1 = re.sub(r'today-project\.pem', '<<DEPLOY_KEY>>', n1) / changed = sum(1 for a,b in zip(t.split('\n'), n1.split('\n')) if a!=b) / with open(p, 'w', encoding='utf-8') as f: f.write(n1) / print(f'masked lines: {changed}') / "
...
- P1(감독) 예측→전술 설계 즉시 / P2(팬) 매치→예측→전술판 자연 흐름 /
  ...
  → 전원 PASS
- 2026-05-29 11:02:43 | git push origin main && git status --short
- 2026-05-29 11:23:49 | git log --oneline -3 -- templates/index.html
- 2026-05-29 12:46:31 | until curl -s -o /dev/null -w '%{http_code}' http://127.0.0.1:5000/health | grep -q 200; do sleep 2; done && echo "local up" / echo "" / echo "=== setpiece-efficiency: suwon vs cheonan ===" / curl -s "http://127.0.0.1:5000/api/setpiece-efficiency?home_slug=suwon&away_slug=cheonan" | python -m json.tool | head -80 / echo "" / echo "=== timing ===" / for i in 1 2 3; do /   curl -s -o NUL -w "run$i ttfb=%{time_starttransfer}s size=%{size_download}\n" "http://127.0.0.1:5000/api/setpiece-efficiency?home_slug=suwon&away_slug=cheonan" / done / echo "" / echo "=== validation ===" / curl -s "http://127.0.0.1:5000/api/setpiece-efficiency?home_slug=&away_slug=cheonan" -w " status=%{http_code}\n" / curl -s "http://127.0.0.1:5000/api/setpiece-efficiency?home_slug=ulsan&away_slug=jeonbuk" | python -c " / import sys, json / d = json.load(sys.stdin) / print('K1 ulsan vs jeonbuk ready:', d.get('ready'), '/ league:', d.get('league')) / if d.get('ready'): /   for side in ('home','away'): /     s = d[side]['by_type'] /     print(f'  {side}: corner shots={s[\"corner\"][\"shots\"]} goals={s[\"corner\"][\"goals\"]}, FK shots={s[\"free-kick\"][\"shots\"]}, PK shots={s[\"penalty\"][\"shots\"]}') / "
- 2026-05-29 12:50:57 | curl -s "http://127.0.0.1:5000/api/setpiece-efficiency?home_slug=ulsan&away_slug=jeonbuk" | python -c " / import sys, json / d = json.load(sys.stdin) / print('ready:', d.get('ready'), '/ league:', d.get('league'), '/ year:', d.get('year')) / for side in ('home','away'): /     s = d[side]['by_type'] /     print(f'  {side}({d[side][\"short\"]}):') /     for k in ('corner','free-kick','penalty'): /         v = s[k] /         print(f'    {k}: shots={v[\"shots\"]} goals={v[\"goals\"]} xg={v[\"xg\"]} conv={v[\"conversion_pct\"]}% xg_diff={v[\"xg_diff\"]}') /     print(f'    total: {d[side][\"total\"]}') / print('league_avg.total:', d['league_avg']['total']) / "
- 2026-05-29 12:52:57 | curl -s "http://127.0.0.1:5000/api/cache-invalidate" -o NUL -w "invalidate=%{http_code}\n" / sleep 2 / echo "--- K1 (xG missing) ---" / curl -s "http://127.0.0.1:5000/api/setpiece-efficiency?home_slug=ulsan&away_slug=jeonbuk" | python -c " / import sys, json / d = json.load(sys.stdin) / for side in ('home','away'): /     s = d[side]['by_type'] /     print(f'  {side}({d[side][\"short\"]}): corner xg={s[\"corner\"][\"xg\"]} xg_diff={s[\"corner\"][\"xg_diff\"]} conv={s[\"corner\"][\"conversion_pct\"]}%') /     print(f'    total xg={d[side][\"total\"][\"xg\"]} xg_diff={d[side][\"total\"][\"xg_diff\"]}') / " / echo "--- K2 (xG present) ---" / curl -s "http://127.0.0.1:5000/api/setpiece-efficiency?home_slug=suwon&away_slug=cheonan" | python -c " / import sys, json / d = json.load(sys.stdin) / for side in ('home','away'): /     s = d[side]['by_type'] /     print(f'  {side}({d[side][\"short\"]}): corner xg={s[\"corner\"][\"xg\"]} xg_diff={s[\"corner\"][\"xg_diff\"]} conv={s[\"corner\"][\"conversion_pct\"]}%') / "
- 2026-05-29 12:53:17 | git status --short

## 2026-05-29 세트피스 효율 카드 (코너/프리킥/PK 전환율 분석)
- **기능**: 매치 상세 예측 패널에 세트피스 효율 카드 추가. 양 팀의 코너/프리킥/PK별 슛·득점·전환율을 리그 평균과 대비하고 xG 효율 차이를 칩으로 표시.
- **백엔드**: `_setpiece_team_season` / `_setpiece_league_avg` / `_setpiece_total` / `_setpiece_total_avg` 헬퍼 + `/api/setpiece-efficiency` 라우트(@cached_response 3600s). match_shotmap.situation IN (corner/free-kick/penalty) 그룹핑, 파라미터 바인딩 준수.
- **xG 비대칭 처리**: K1 2026은 shotmap.xg 전부 NULL → xg_known 카운트로 has_xg 판정, 미백필 리그는 xg/xg_per_shot/xg_diff = None. 프론트에서 xgChip() 조건부 렌더(K1은 전환율 기반 인사이트로 대체).
- **프론트**: prediction.js `renderSetpieceEffCard()` + _speCache, Promise.all 7번째 요소로 speFetch 합류. CSS .pred-setpiece-eff 막대 시각화(--w/--c 변수, spe-bar-grow 애니), 리그 평균선, xG 칩, 700px 반응형.
- **캐시버스트**: prediction.js v=49, style.css v=41 (index.html 동반 증가).
...
- 2026-06-02 08:53:32 | curl -s --max-time 20 http://<IP-REDACTED>/api/update-status; echo; echo "---HTTPS---"; curl -sk --max-time 20 https://<IP-REDACTED>/api/update-status
- 2026-06-02 08:54:25 | ls -la ~/.ssh/ 2>/dev/null; echo "---"; find "C:/Users/ehban" -maxdepth 3 -iname "*.pem" 2>/dev/null | head; echo "---keys in repo---"; ls *.pem 2>/dev/null
- 2026-06-02 09:03:04 | KEY="C:/Users/ehban/.ssh/<KEY-REDACTED>" / ssh -i "$KEY" -o StrictHostKeyChecking=no -o IdentitiesOnly=yes -o ConnectTimeout=15 rocky@<IP-REDACTED> ' / cd /opt/today_tactics / venv/bin/python - <<"PYEOF" / import sqlite3 / c=sqlite3.connect("players.db");cur=c.cursor() / for t in ["events","match_player_stats","heatmap_points"]: /     cur.execute(f"PRAGMA table_info({t})") /     cols=[r[1] for r in cur.fetchall()] /     print(f"{t}:") /     print("  "+", ".join(cols)) /     print() / c.close() / PYEOF / ' 2>&1 | grep -vE "post-quantum|store now|openssh.com|Warning: Perm"
...
- 2026-06-02 15:32:38 | KEY="C:/Users/ehban/.ssh/<KEY-REDACTED>" / ssh -i "$KEY" -o StrictHostKeyChecking=no -o IdentitiesOnly=yes -o ConnectTimeout=15 rocky@<IP-REDACTED> 'bash -lc " / cd /opt/today_tactics / venv/bin/python - <<PYEOF / import sqlite3 / c=sqlite3.connect(\"players.db\");cur=c.cursor() / y0,y1=\"2026-01-01\",\"2027-01-01\" / team=dict(cur.execute(\"\"\"SELECT t.id, COALESCE(SUM(CASE WHEN c.card_type IN(\"yellow\",\"yellowRed\") THEN 1 ELSE 0 END),0) /  FROM teams t JOIN events e ON (e.home_team_id=t.id OR e.away_team_id=t.id) /  LEFT JOIN card_events c ON c.event_id=e.id AND c.team_id=t.id AND c.player_id IS NOT NULL /  WHERE e.home_score IS NOT NULL AND e.id<50000000 AND e.date_ts>=strftime(\"%s\",?) AND e.date_ts<strftime(\"%s\",?) GROUP BY t.id\"\"\",(y0,y1)).fetchall()) / psum={} / for tid,yc in cur.execute(\"\"\"SELECT c.team_id, SUM(CASE WHEN c.card_type IN(\"yellow\",\"yellowRed\") THEN 1 ELSE 0 END) /  FROM card_events c JOIN events e ON c.event_id=e.id /  WHERE c.player_id IS NOT NULL AND e.id<50000000 AND e.home_score IS NOT NULL AND e.date_ts>=strftime(\"%s\",?) AND e.date_ts<strftime(\"%s\",?) GROUP BY c.team_id\"\"\",(y0,y1)).fetchall(): psum[tid]=yc / mism=sum(1 for tid,tv in team.items() if (tv or 0)!=(psum.get(tid,0) or 0) and (tv or psum.get(tid,0))) / checked=sum(1 for tid,tv in team.items() if (tv or psum.get(tid,0))) / print(f\"SERVER verify: {checked} teams, mismatches={mism}\") / c.close() / PYEOF / "' 2>&1 | grep -vE "post-quantum|store now|openssh.com|Warning: Perm"
...
- 2026-06-04 14:48:01 | ssh -i "$HOME/.ssh/<KEY-REDACTED>" -o StrictHostKeyChecking=accept-new rocky@<IP-REDACTED> ' / sudo mkdir -p /var/www/certbot/.well-known/acme-challenge / echo "tt-precheck-ok" | sudo tee /var/www/certbot/.well-known/acme-challenge/tt-precheck >/dev/null / sudo chown -R root:root /var/www/certbot / for h in today-football-tactics.xyz www.today-football-tactics.xyz; do /   printf "%s -> " "$h"; curl -s -o /dev/null -w "HTTP %{http_code}  " "http://$h/.well-known/acme-challenge/tt-precheck" /   curl -s "http://$h/.well-known/acme-challenge/tt-precheck"; echo / done / sudo rm -f /var/www/certbot/.well-known/acme-challenge/tt-precheck / '
- 2026-06-04 14:48:20 | ssh -i "$HOME/.ssh/<KEY-REDACTED>" -o StrictHostKeyChecking=accept-new rocky@<IP-REDACTED> ' / sudo certbot certonly --webroot -w /var/www/certbot \ /   -d today-football-tactics.xyz -d www.today-football-tactics.xyz \ /   --non-interactive --agree-tos -m <EMAIL-REDACTED> --keep-until-expiring / echo "=== exit: $? ===" / echo "=== live dir ==="; sudo ls -l /etc/letsencrypt/live/today-football-tactics.xyz/ 2>/dev/null / '
- 2026-06-04 14:49:11 | ssh -i "$HOME/.ssh/<KEY-REDACTED>" -o StrictHostKeyChecking=accept-new rocky@<IP-REDACTED> ' / echo "--- canonical www verbose ---" / curl -sS -o /dev/null -w "HTTP %{http_code}\n" https://www.today-football-tactics.xyz/ 2>&1 / echo "--- apex ---" / curl -sS -o /dev/null -w "HTTP %{http_code} loc=%{redirect_url}\n" https://today-football-tactics.xyz/ 2>&1 / echo "--- old domain ---" / curl -sS -o /dev/null -w "HTTP %{http_code} loc=%{redirect_url}\n" https://today-tactics.co.kr/ 2>&1 / echo "--- local backend ---" / curl -sS -o /dev/null -w "HTTP %{http_code}\n" http://127.0.0.1:5000/ 2>&1 / '
- 2026-06-05 10:12:54 | ls -la ~/.ssh/<SSH_KEY>.pem 2>&1 | head -2; echo "---conn test---"; ssh -i ~/.ssh/<SSH_KEY>.pem -o StrictHostKeyChecking=accept-new -o ConnectTimeout=12 -o BatchMode=yes rocky@<PROD_IP> "echo OK; cd /opt/today_tactics && git rev-parse --short HEAD 2>/dev/null; whoami" 2>&1 | head -10
- 2026-06-05 10:13:17 | ssh -i ~/.ssh/<SSH_KEY>.pem -o ConnectTimeout=12 -o BatchMode=yes rocky@<PROD_IP> "cd /opt/today_tactics && echo '=== git status ==='; git status --short | head; echo '=== branch/head ==='; git log --oneline -1; echo '=== deployed index.html version markers ==='; grep -oE 'style.css\?v=[0-9]+|analytics.js\?v=[0-9]+' templates/index.html; echo '=== analytics.js has new fn? ==='; grep -c 'renderMargin\|leagueRank' static/js/analytics.js" 2>&1 | grep -v "WARNING\|vulnerable\|may need\|openssh.com\|^\*\*\|^$"
- 2026-06-05 10:13:43 | ssh -i ~/.ssh/<SSH_KEY>.pem -o ConnectTimeout=12 -o BatchMode=yes rocky@<PROD_IP> "sudo -n systemctl restart today_tactics 2>&1 && echo 'RESTART_OK' || echo 'NEEDS_PASSWORD'; sleep 2; systemctl is-active today_tactics; curl -s -o /dev/null -w 'HTTP %{http_code}\n' http://127.0.0.1/" 2>&1 | grep -v "WARNING\|vulnerable\|may need\|openssh.com\|^\*\*\|^$"
- 2026-06-05 10:18:43 | ssh -i ~/.ssh/<SSH_KEY>.pem -o ConnectTimeout=15 -o BatchMode=yes rocky@<PROD_IP> "cd /opt/today_tactics && git fetch origin main -q && git reset --hard origin/main -q && git log --oneline -1 && sudo -n systemctl restart today_tactics && sleep 2 && systemctl is-active today_tactics" 2>&1 | grep -v "WARNING\|vulnerable\|may need\|openssh.com\|^\*\*\|^$"
- 2026-06-05 10:19:33 | curl -sL "https://www.today-football-tactics.xyz/" 2>/dev/null | python -c " / import sys / html=sys.stdin.read() / i=html.find('\U0001f6e1') / if i<0: /     print('shield(U+1F6E1) not present in live HTML') / else: /     frag=html[i:i+3] /     print('found shield at',i,'next codepoints:', [hex(ord(c)) for c in frag]) /     print('VS16 present right after:', ord(html[i+1])==0xfe0f) / print('ws-tab-icon count:', html.count('ws-tab-icon')) / "
- 2026-06-05 10:19:44 | curl -sL "https://www.today-football-tactics.xyz/" 2>/dev/null | python -c " / import sys,re / html=sys.stdin.read() / for mm in re.finditer(r'ws-tab-icon[^>]*>(.*?)</span>', html): /     s=mm.group(1) /     print(repr(s), [hex(ord(c)) for c in s]) / "
- 2026-06-05 10:20:00 | curl -sL "https://www.today-football-tactics.xyz/" -o /tmp/live.html 2>/dev/null; PYTHONIOENCODING=utf-8 python -c " / import re / html=open('/tmp/live.html','rb').read().decode('utf-8','replace') / icons=re.findall(r'ws-tab-icon[^>]*>(.*?)</span>', html) / for s in icons: /     print([hex(ord(c)) for c in s], 'VS16' if 0xfe0f in [ord(c) for c in s] else '') / print('shield present:', '\U0001f6e1' in html) / "; rm -f /tmp/live.html
- 2026-06-05 10:20:58 | curl -sL "https://www.today-football-tactics.xyz/" 2>/dev/null | PYTHONIOENCODING=utf-8 python -c " / import sys,re / html=sys.stdin.buffer.read().decode('utf-8','replace') / icons=re.findall(r'ws-tab-icon[^>]*>(.*?)</span>', html) / for s in icons: /     cps=[ord(c) for c in s] /     print([hex(c) for c in cps], '<-- VS16' if 0xfe0f in cps else '') / print('shield U+1F6E1 present:', '\U0001f6e1' in html) / "
- 2026-06-05 10:35:52 | ssh -i ~/.ssh/<SSH_KEY>.pem -o ConnectTimeout=15 -o BatchMode=yes rocky@<PROD_IP> "cd /opt/today_tactics && python3 -c \" / import sqlite3 / c=sqlite3.connect('players.db') / def q(s): return c.execute(s).fetchall() / r=q(\\\"SELECT COUNT(*), SUM(CASE WHEN home_score IS NOT NULL THEN 1 ELSE 0 END), MAX(CASE WHEN home_score IS NOT NULL THEN datetime(date_ts,'unixepoch','localtime') END) FROM events WHERE tournament_id=410 AND strftime('%Y',datetime(date_ts,'unixepoch','localtime'))='2026'\\\")[0] / print('PROD K1 2026: total=%s played=%s last_played=%s'%r) / m=q(\\\"SELECT COUNT(DISTINCT mps.event_id) FROM match_player_stats mps JOIN events e ON e.id=mps.event_id WHERE e.tournament_id=410 AND strftime('%Y',datetime(e.date_ts,'unixepoch','localtime'))='2026'\\\")[0][0] / print('PROD mps events:',m) / \"" 2>&1 | grep -v "WARNING\|vulnerable\|may need\|openssh.com\|^\*\*\|^$"
- 2026-06-05 12:43:34 | ssh -i ~/.ssh/<SSH_KEY>.pem -o ConnectTimeout=15 -o BatchMode=yes rocky@<PROD_IP> "cd /opt/today_tactics && git fetch origin main -q && git reset --hard origin/main -q && git log --oneline -1 && sudo -n systemctl restart today_tactics && sleep 3 && systemctl is-active today_tactics && curl -s -o /dev/null -w 'local HTTP %{http_code}\n' http://127.0.0.1/" 2>&1 | grep -v "WARNING\|vulnerable\|may need\|openssh.com\|^\*\*\|^$"
- 2026-06-05 12:55:20 | ssh -i ~/.ssh/<SSH_KEY>.pem -o ConnectTimeout=15 -o BatchMode=yes rocky@<PROD_IP> "cd /opt/today_tactics && git fetch origin main -q && git reset --hard origin/main -q && git log --oneline -1 && sudo -n systemctl restart today_tactics && sleep 3 && systemctl is-active today_tactics" 2>&1 | grep -v "WARNING\|vulnerable\|may need\|openssh.com\|^\*\*\|^$"
- 2026-06-05 13:05:59 | ssh -i ~/.ssh/<SSH_KEY>.pem -o ConnectTimeout=15 -o BatchMode=yes rocky@<PROD_IP> "ps aux | grep '[g]it' | head; echo '--- lock age ---'; ls -la /opt/today_tactics/.git/index.lock 2>/dev/null; echo '--- date ---'; date" 2>&1 | grep -v "WARNING\|vulnerable\|may need\|openssh.com\|^\*\*\|^$"
- 2026-06-05 13:06:42 | ssh -i ~/.ssh/<SSH_KEY>.pem -o ConnectTimeout=15 -o BatchMode=yes rocky@<PROD_IP> "cd /opt/today_tactics && rm -f .git/index.lock && git fetch origin main -q && git reset --hard origin/main -q && git log --oneline -1 && sudo -n systemctl restart today_tactics && sleep 4 && systemctl is-active today_tactics" 2>&1 | grep -v "WARNING\|vulnerable\|may need\|openssh.com\|^\*\*\|^$"
- 2026-06-05 13:08:13 | ssh -i ~/.ssh/<SSH_KEY>.pem -o ConnectTimeout=15 -o BatchMode=yes rocky@<PROD_IP> "cd /opt/today_tactics; echo HEAD:; git log --oneline -1; echo STATUS:; git status --short; echo SERVICE:; systemctl is-active today_tactics" 2>&1 | grep -v "WARNING\|vulnerable\|may need\|openssh.com\|^\*\*\|^$"
- 2026-06-09 13:05:36 | ssh -i <SSH_KEY>.pem -o IdentitiesOnly=yes -o StrictHostKeyChecking=no -o ConnectTimeout=20 rocky@<PROD_IP> 'cd /opt/today_tactics && echo "HEAD: $(git log --oneline -1)" && echo "SERVICE: $(systemctl is-active today_tactics)" && echo "HEALTH: $(curl -sf -o /dev/null -w %{http_code} http://127.0.0.1:5000/)" && python3 -c " / import sqlite3,datetime / c=sqlite3.connect(\"players.db\") / ids=[15403892,15403895,15403896,15403894,15403897,15403898,15403899,15403912] / print(\"--- PROD DB: 8 K2 matches ---\") / for eid in ids: /     e=c.execute(\"SELECT home_team_name,away_team_name,home_score,away_score FROM events WHERE id=?\",(eid,)).fetchone() /     mps=c.execute(\"SELECT COUNT(*) FROM match_player_stats WHERE event_id=?\",(eid,)).fetchone()[0] /     hm=c.execute(\"SELECT COUNT(*) FROM heatmap_points WHERE event_id=?\",(eid,)).fetchone()[0] /     print(eid, (e[0],e[2],e[3],e[1]) if e else \"NO EVENT\", \"mps=%d hm=%d\"%(mps,hm)) / "' 2>&1 | grep -v -E "Warning|Permanently|known_hosts"
- 2026-06-09 13:06:43 | ssh -i <SSH_KEY>.pem -o IdentitiesOnly=yes -o StrictHostKeyChecking=no -o ConnectTimeout=20 rocky@<PROD_IP> 'cd /opt/today_tactics && ls crawlers/fetch_event_heatmap.py && python3 crawlers/fetch_event_heatmap.py --help 2>&1 | head -25' 2>&1 | grep -v -E "WARNING|vulnerable|may need|openssh.com|post-quantum|store now|Permanently|known_hosts|^\*\*"
- 2026-06-09 13:06:56 | ssh -i <SSH_KEY>.pem -o IdentitiesOnly=yes -o StrictHostKeyChecking=no -o ConnectTimeout=20 rocky@<PROD_IP> 'cd /opt/today_tactics && for p in venv .venv env ../venv; do if [ -x "$p/bin/python3" ]; then echo "FOUND: $p"; "$p/bin/python3" -c "import playwright; print(\"  playwright OK\")" 2>&1 | head -1; fi; done; echo "--- service ExecStart ---"; systemctl cat today_tactics 2>/dev/null | grep -E "ExecStart|WorkingDirectory|Environment"' 2>&1 | grep -v -E "WARNING|vulnerable|may need|openssh.com|post-quantum|store now|Permanently|known_hosts|^\*\*"
- 2026-06-09 13:08:59 | ssh -i <SSH_KEY>.pem -o IdentitiesOnly=yes -o StrictHostKeyChecking=no -o ConnectTimeout=30 rocky@<PROD_IP> 'cd /opt/today_tactics && timeout 240 venv/bin/python3 crawlers/fetch_event_heatmap.py --days 14 2>&1 | tail -30' 2>&1 | grep -v -E "WARNING|vulnerable|may need|openssh.com|post-quantum|store now|Permanently|known_hosts|^\*\*"
- 2026-06-09 13:09:16 | ssh -i <SSH_KEY>.pem -o IdentitiesOnly=yes -o StrictHostKeyChecking=no -o ConnectTimeout=20 rocky@<PROD_IP> 'cd /opt/today_tactics && python3 -c " / import sqlite3 / c=sqlite3.connect(\"players.db\") / ids=[15403892,15403895,15403896,15403894,15403897,15403898,15403899,15403912] / allok=True / for eid in ids: /     e=c.execute(\"SELECT home_team_name,away_team_name FROM events WHERE id=?\",(eid,)).fetchone() /     mps=c.execute(\"SELECT COUNT(*) FROM match_player_stats WHERE event_id=?\",(eid,)).fetchone()[0] /     hm=c.execute(\"SELECT COUNT(*) FROM heatmap_points WHERE event_id=?\",(eid,)).fetchone()[0] /     flag=\"OK\" if (mps>0 and hm>0) else \"GAP\" /     if flag==\"GAP\": allok=False /     print(\"%s %-12s vs %-22s mps=%d hm=%d\"%(flag,e[0],e[1],mps,hm)) / print(\"=== ALL COMPLETE ===\" if allok else \"=== STILL HAS GAP ===\") / "' 2>&1 | grep -v -E "WARNING|vulnerable|may need|openssh.com|post-quantum|store now|Permanently|known_hosts|^\*\*"
- 2026-06-09 13:10:35 | ssh -i <SSH_KEY>.pem -o IdentitiesOnly=yes -o StrictHostKeyChecking=no -o ConnectTimeout=20 rocky@<PROD_IP> 'sudo -n systemctl restart today_tactics && sleep 4 && echo "SERVICE: $(systemctl is-active today_tactics)" && echo "HEALTH: $(curl -sf -o /dev/null -w %{http_code} http://127.0.0.1:5000/)" && echo "HEATMAP_API: $(curl -sf -o /dev/null -w %{http_code} "http://127.0.0.1:5000/api/heatmap?event_id=15403899" 2>/dev/null)"' 2>&1 | grep -v -E "WARNING|vulnerable|may need|openssh.com|post-quantum|store now|Permanently|known_hosts|^\*\*"
- 2026-06-09 13:10:52 | ssh -i <SSH_KEY>.pem -o IdentitiesOnly=yes -o StrictHostKeyChecking=no -o ConnectTimeout=20 rocky@<PROD_IP> 'cd /opt/today_tactics && pid=$(python3 -c "import sqlite3;c=sqlite3.connect(\"players.db\");print(c.execute(\"SELECT player_id FROM heatmap_points WHERE event_id=15403899 LIMIT 1\").fetchone()[0])") && echo "test player_id=$pid" && curl -s -o /dev/null -w "HTTP=%{http_code} bytes=%{size_download}\n" "http://127.0.0.1:5000/api/heatmap?player_id=$pid&event_id=15403899"' 2>&1 | grep -v -E "WARNING|vulnerable|may need|openssh.com|post-quantum|store now|Permanently|known_hosts|^\*\*"
...
- 2026-06-09 15:34:38 | for i in $(seq 1 8); do h=$(ssh -i <SSH_KEY>.pem -o IdentitiesOnly=yes -o StrictHostKeyChecking=no -o ConnectTimeout=15 rocky@<PROD_IP> 'cd /opt/today_tactics && git rev-parse --short HEAD' 2>/dev/null | tr -d "[:space:]"); echo "try $i: prod HEAD=$h"; if [ "$h" = "c1be6e2" ]; then echo "SYNCED"; break; fi; sleep 8; done
