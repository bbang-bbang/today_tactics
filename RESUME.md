...
- **브랜치/HEAD**: `main` `4ed9194` (origin 동기화됨, 작업트리 clean). 최신 작업 = **선수 배너 인사이트 전면 개편** + 팀 배너 라이트잔재 다크화 + 심화 인사이트 4종(날씨·승부처·폼·활동량) + 카드 집계 정합성 + 버퍼(성능) 해소.
- **정적 자산 최신 버전**: `insights.js?v=18`, `style.css?v=63` (변경 시 `templates/index.html`의 `?v=` 동반 증가 필수).
