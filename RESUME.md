...
> 최종 갱신: 2026-06-12 (history: `checklist/history.md` 2026-06-12 섹션 — 세부 포지션 비교 필터)

---

## 0. ⏭ 재개 지점 (NEXT)
**열린 작업 없음.** 직전 보류건(히트맵 비교 필터 세분화)은 2026-06-12 **B안으로 완료·배포**됨:
- `match_lineups.detail_pos`(formation+slot 결정론적 유도, 재크롤링 불필요) → 비교 필터 8그룹(GK/CB/FB/DM/CM/AM/W/ST). 로컬=origin=prod `12ab462`, 라이브 200 검증. prod DB도 백필 완료(SSH 1회 + `update_data.py` STEP 18 자동화).
- 다음 작업은 §4 백로그 참조.

## 1. 현재 상태 (2026-06-11 종료 시점)
- **브랜치/HEAD**: `main` (2026-06-15 심화 ③스카우팅·⑤경기리포트·④선수비교 완료). **origin push + 운영 git 동기화 완료**(로컬=origin=prod 일치 · 서비스 active · 헬스 200). 로컬 트리는 `checklist/history.md` 자동로그(마스킹 훅)로 수시 dirty(감사 로그 — 배포 무관).
...
- **정적 자산 최신 버전**: `analytics.js?v=13` · `prediction.js?v=60` · `match_report.js?v=1` · `player_compare.js?v=1` · `global_league.js?v=1` · `k2heatmap.js?v=16` · `player_report.js?v=6` · `insights.js?v=38` · `workspace.js?v=7` · `style.css?v=92`. 변경 시 `templates/index.html`의 `?v=` 동반 증가 필수.
...
- 단일 팀 분석 모달(`#team-analysis-modal`) = **6탭**: 📋 결과 분석 / 🎯 스카우팅 / 📈 심화 인사이트 / 🧭 팀 형태 / 📍 슛맵 / 📡 스킬 프로필. `analytics.js`가 7개 엔드포인트(team-analytics/trend/league-rankings/team-insights/team-shape/team-shotmap×for·against) Promise.all 오케스트레이션. **🎯 스카우팅은 신규 API 없이** shape+shotmap을 `renderScout`로 조합(경계할 점·공략 포인트·게임플랜).
