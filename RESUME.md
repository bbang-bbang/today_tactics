...
- **브랜치/HEAD**: `main` `3120af4` (origin 동기화됨, 작업트리 clean). 최신 작업 = **선수 배너 인사이트 전면 재설계** — 탭 분리(랭킹/심화/규율)·위계(부문 카드→슬림 표)·합성지표(공격P/창출P/수비P)+리그평균 기준선·승부처 결정적 골·심화 4종(날씨/승부처/폼/활동량)·팀 배너 다크화·카드 집계 정합성·다수 성능 최적화.
- **정적 자산 최신 버전**: `insights.js?v=27`, `style.css?v=70`, `team_compare.js?v=12` (변경 시 `templates/index.html`의 `?v=` 동반 증가 필수 — [[feedback_cache_bust_static]]).
- **운영**: https://today-tactics.co.kr 라이브. `git push` 후 서버 `git reset --hard origin/main` + `systemctl restart today_tactics` + 라이브 검증으로 반영. (이번 세션은 매 커밋 수동 동기화)
  - ⚠️ 재시작 직후 ~60~90초는 워밍업이 2vCPU 점유 → 일시 502/지연. 정상 현상(자정 자동 재시작이라 사용자 영향 적음).
- **데이터**: 서버가 **매주 월 05:00 cron `update_data.py` 자체 수집**. 최신 경기 5/31(R14)까지, 5/31 3경기 히트맵은 본 세션에서 수동 백필 완료.
- **세션 규모**: 약 27커밋(`e7401e9 → 3120af4`). 상세는 work-log-2026-06-02.md (오전 정리 + 오후 인사이트 심화 개편 두 파트).
